...
---
flowchart LR
    ED[External Device] --> Gateway
    Gateway -->|Port forward| n1[Node 1]
    Gateway -->|Port forward| n2[Node 2]
    Gateway -->|Port forward| n3[Node 3]
    subgraph Home LAN
        Gateway
        n1
        n2
...
2. In the `nat` table we need a `MASQUERADE` rule on the `POSTROUTING` chain to translate the source IP on the forwarded packets from the Laptop's Wireguard IP to the Wireguard server's Server LAN IP

(1) is fairly self-explanatory. We need (2) because if we forward packets from the Wireguard server to Node 2 or Node 3 without it, the nodes will have no idea where to respond. Remember, with the Hub and Spoke architecture the only device with a Wireguard interface on our Home LAN is Node 1, our Wireguard server. This means for Nodes 2 & 3 to be able to respond back to the Wireguard interface on the laptop, The responses will have to be routed back through Node 1 which is on the Wireguard network. The `MASQUERADE` rule translates the source IP from the laptop's Wireguard IP to Node 1's Server LAN IP. It tracks that translation in the kernel's connection tracking table and will translate back to the original address once it receives a response through that connection.

<pre class="mermaid" style="text-align: center;">
---
title: Successful hub-and-spoke request
---
sequenceDiagram
    participant EC as External Client
    participant WS as Wireguard Server
    participant N1 as Server LAN Node
    EC->>WS: Src 192.168.100.10
    WS->>N1: Src 192.168.2.1 (masq)
    activate WS
    N1-->>WS: Dest 192.168.2.1 (masq)
    deactivate WS
...
<pre class="mermaid" style="text-align: center;">
---
title: Failed hub-and-spoke request
---
sequenceDiagram
    participant EC as External Client
    participant WS as Wireguard Server
...
flowchart LR
    GW["Gateway
    (wg server)"]
    ED[External Device] --> GW
    ED2[External Device 2] --> GW
